UPDATE `{#}comments` SET `author_name` = 'Гість' WHERE `author_name` IS NOT NULL;
UPDATE `{#}widgets` SET `controller` = 'auth', `title` = 'Форма авторизації' WHERE `controller` IS NULL AND `name` = 'auth';
