...
function install_package(){
    ...
    if(!$core->db->getRowsCount('activity_types', "controller = 'subscriptions' AND name = 'subscribe'")){
        $core->db->query("INSERT INTO `{#}activity_types` (`is_enabled`, `controller`, `name`, `title`, `description`) VALUES (1, 'subscriptions', 'subscribe', 'Підписка на контент', 'підписується на список %s');");
    }

    $core->db->query("ALTER TABLE `{#}controllers` CHANGE `files` `files` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT 'Список файлів контролера (для сторонніх компонентів)';");
    $core->db->query("ALTER TABLE `{#}widgets` CHANGE `files` `files` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT 'Список файлів віджету (для сторонніх віджетів)';");
    ...
    if(!$core->db->getRowsCount('scheduler_tasks', "controller = 'subscriptions' AND hook = 'delete_expired_unconfirmed'")){
        $core->db->query("INSERT INTO `{#}scheduler_tasks` (`title`, `controller`, `hook`, `period`, `is_strict_period`, `date_last_run`, `is_active`) VALUES ('Видаляє прострочені непідтверджені підписки гостей', 'subscriptions', 'delete_expired_unconfirmed', 1440, 1, '2018-03-21 00:03:00', 1);");
    }

    if(!$core->db->getRowsCount('controllers', "name = 'subscriptions'")){
        $core->db->query("INSERT INTO `{#}controllers` (`title`, `name`, `slug`, `is_enabled`, `options`, `author`, `url`, `version`, `is_backend`, `is_external`, `files`, `addon_id`) VALUES ('Підписки', 'subscriptions', NULL, 1, '---\nguest_email_confirmation: 1\nneed_auth: null\nverify_exp: 24\nupdate_user_rating: 1\nrating_value: 1\nadmin_email:\nlimit: 20\n', 'InstantCMS Team', 'http://www.instantcms.ru', '2.0', 1, NULL, NULL, NULL);");
    }

    if(!$core->db->getRowsCount('widgets', "controller = 'groups' AND name = 'list'")){
        $core->db->query("INSERT INTO `{#}widgets` (`controller`, `name`, `title`, `author`, `url`, `version`, `is_external`) VALUES ('groups', 'list', 'Список груп', 'InstantCMS Team', 'http://www.instantcms.ru', '2.0', NULL);");
    }

    if(!$core->db->getRowsCount('widgets', "controller = 'subscriptions' AND name = 'button'")){
        $core->db->query("INSERT INTO `{#}widgets` (`controller`, `name`, `title`, `author`, `url`, `version`, `is_external`) VALUES ('subscriptions', 'button', 'Кнопки підписки', 'InstantCMS Team', 'http://www.instantcms.ru', '2.0', NULL);");
    }

    if(!$core->db->getRowsCount('widgets', "controller = 'auth' AND name = 'register'")){
        $core->db->query("INSERT INTO `{#}widgets` (`controller`, `name`, `title`, `author`, `url`, `version`, `is_external`) VALUES ('auth', 'register', 'Форма реєстрації', 'InstantCMS Team', 'http://www.instantcms.ru', '2.0', NULL);");
    }
